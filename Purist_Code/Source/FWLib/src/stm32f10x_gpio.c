...
void GPIO_PinRemapConfig(uint32_t GPIO_Remap, FunctionalState NewState)
{
  ...
  tmpmask = (GPIO_Remap & DBGAFR_POSITION_MASK) >> 0x10; // ylf: get pos in map regs
  tmp = GPIO_Remap & LSB_MASK; // ylf: low 16 bits

  if ((GPIO_Remap & (DBGAFR_LOCATION_MASK | DBGAFR_NUMBITS_MASK)) == (DBGAFR_LOCATION_MASK | DBGAFR_NUMBITS_MASK))
  {
    tmpreg &= DBGAFR_SWJCFG_MASK; // YLF: clear SWJ
    AFIO->MAPR &= DBGAFR_SWJCFG_MASK;
  }
  else if ((GPIO_Remap & DBGAFR_NUMBITS_MASK) == DBGAFR_NUMBITS_MASK) // YLF: I.E. GPIO_PartialRemap_USART3
  {
    tmp1 = ((uint32_t)0x03) << tmpmask; // ylf: have position then got mask 
    tmpreg &= ~tmp1; // ylf: clean first in our position
  ...
  if (NewState != DISABLE)
  {
    tmpreg |= (tmp << ((GPIO_Remap >> 0x15)*0x10)); // ylf: set value
  }
