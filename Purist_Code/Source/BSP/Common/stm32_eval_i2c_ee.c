...
void sEE_WriteByte(uint8_t* pBuffer, uint16_t WriteAddr)
{
  ...
  I2C_Send7bitAddress(sEE_I2C, sEE_GetIICAddress(WriteAddr), I2C_Direction_Transmitter);
  ...
  I2C_SendData(sEE_I2C, sEE_GetDataAddress(WriteAddr));
...
{  
  ...
  /*!< Send EEPROM address for write */
  I2C_Send7bitAddress(sEE_I2C, sEE_GetIICAddress(ReadAddr), I2C_Direction_Transmitter);
  ...
  I2C_SendData(sEE_I2C, sEE_GetDataAddress(ReadAddr));  
  ...
  if ((uint16_t)(*NumByteToRead) < 2)
  {
    ...
    I2C_AcknowledgeConfig(sEE_I2C, DISABLE); // ylf: no ack for last byte according at24c04
...
void sEE_WaitEepromStandbyState(void)      
{
  ...
  }while(!(I2C_ReadRegister(sEE_I2C, I2C_Register_SR1) & 0x0002));// ylf: wait until address sent
